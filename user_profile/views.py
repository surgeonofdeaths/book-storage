...
class ProfileEditView(DataMixin, LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def post(self, request, slug, *args, **kwargs):
        ...
        profile_form.fields.get('avatar').widget.clear_checkbox_label = 'Remove Image'
        profile_form.fields.get('avatar').widget.input_text = 'Change Avatar'
        if user_form.is_valid() and profile_form.is_valid():
            profile_form.save()
            user_form.save()
            messages.success(request, 'Your profile has been updated!')
            print('user_form: ', user_form.cleaned_data)
            print('profile_form: ', profile_form.cleaned_data)
        ...
        # return redirect(reverse('edit_profile', kwargs={'slug': slug}))
